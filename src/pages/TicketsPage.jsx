...
export default function TicketsPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['support-tickets', page, search],
    queryFn: () => billingService.tickets({
      page,
      per_page: 20,
      ...(search ? { search } : {}),
    }).then(r => r.data),
  ...
  const columns = [
    { key: 'id', label: 'Ticket #' },
    { key: 'subject', label: 'Subject', render: v => <span className="font-medium text-white">{v || '—'}</span> },
    { key: 'department', label: 'Department', render: (_, row) => row.department?.name || row.department || '—' },
    { key: 'client_name', label: 'Client', render: (_, row) => row.client_name || row.client?.email || '—' },
    { key: 'priority', label: 'Priority', render: v => (v ? String(v).toUpperCase() : '—') },
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${meta.total ?? tickets.length} active and resolved tickets`}
